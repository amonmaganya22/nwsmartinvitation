...
export default function ForgotPasswordPage() {
  ...
  return (
    <AuthShell title="Reset your password" subtitle="We'll send reset instructions to your email.">
      {sent ? (
        <div className="text-center">
          <p className="text-sm text-gray-600 dark:text-gray-300">
            If an account exists for <span className="font-medium">{email}</span>, you&apos;ll receive a reset link shortly.
          </p>
          <Link href="/login" className="mt-6 inline-block font-medium text-brand-green">Back to login</Link>
        </div>
      ) : (
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <Link href="/login" className="block text-center text-sm font-medium text-brand-green">Back to login</Link>
        </form>
      )}
    </AuthShell>
  );
